...
void gen_program(A_NODE *node)
{
	...
	case N_PROGRAM:
		...
		gen_label_number(main_start_label);
...
void gen_expression(A_NODE *node)
{
	...
	case N_EXP_IDENT:
		...
		case ID_ENUM_LITERAL:
			gen_code_i(LITI, 0, (int)(long)id->init);
			break;
		// 함수명이 나오면 함수의 주소를 반환
		case ID_FUNC:
	...
	case N_EXP_INT_CONST:
		gen_code_i(LITI, 0, (int)(long)node->clink);
		break;
	case N_EXP_FLOAT_CONST:
		i = (int)(long)node->clink;
	...
	case N_EXP_FUNCTION_CALL:
		...
		if (node->rlink)
		{
			...
			gen_code_i(POP, 0, node->rlink->value / 4 + 2);
		}
		else
			gen_code_i(INT, 0, i);
		gen_code_i(CAL, 0, 0);
	...
	case N_EXP_SUB:
		...
		else if (isPointerOrArrayType(node->llink->type) && isPointerOrArrayType(node->rlink->type))
		{
			gen_code_i(SUBI, 0, 0);
			gen_code_i(LITI, 0, node->type->element_type->size);
			gen_code_i(DIVI, 0, 0);
...
// 초기화 함수
void gen_initializer(A_NODE *node, A_TYPE *t, int level, int addr)
{
	int i = 0;
	...
	{
	case N_INIT_LIST:
		while (node->name != N_INIT_LIST_NIL)
		{
			gen_initializer(node->llink, t, level, addr+i);
	...
	case N_INIT_LIST_ONE:
		gen_code_i(LDA, level, addr);
		gen_expression(node->clink);
		i = t->size;
		if (i == 1)
			gen_code_i(STOB, 0, 0);
		else
			gen_code_i(STO, 0, i%4? i/4+1 : i/4);
		break;
...
void gen_declaration(A_ID *id)
{
	...
	case ID_VAR:
		...
		{
			// 초기화 기능 추가
			if (id->type->kind == T_ARRAY)
				gen_initializer(id->init, id->type->element_type, id->level, id->address);
			else
				gen_initializer(id->init, id->type, id->level, id->address);
			/*
			기존
			if (id->level == 0)
	...
	case ID_FUNC:
		if (id->type->expr)
		{
			// 함수 첫 생성 시 main함수 시작으로 JMP 코드 추가
			if (main_start_label == 0)
				gen_code_l(JMP, 0, main_start_label=get_label());
...
{
	++gen_err;
	fprintf(stderr, "*** code generation error at line %d: ", ll);
	switch (i)
	{
	case 11:
		fprintf(stderr, "illegal identifier in expression\n");
		break;
	case 12:
		fprintf(stderr, "illegal l-value expression\n");
		break;
	case 13:
		fprintf(stderr, "identifier %s not l-value expression\n", s);
		break;
	case 20:
		fprintf(stderr, "illegal default label in switch statement\n");
		break;
	case 21:
		fprintf(stderr, "illegal case label in switch statement\n");
		break;
	case 22:
		fprintf(stderr, "no destination for continue statement\n");
		break;
	case 23:
		fprintf(stderr, "no destination for break statement\n");
		break;
	case 100:
		fprintf(stderr, "fatal compiler error during code generation\n");
		break;
	default:
