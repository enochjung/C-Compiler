...
void prt_sem_expression(A_NODE *node, int s)
{
	...
	case N_EXP_INT_CONST:
		prt_sem_integer((int)(long)node->clink, s+1);
		break;
	case N_EXP_FLOAT_CONST:
		prt_sem_LITERAL((int)(long)node->clink, s+1);
		break;
...
void prt_sem_statement(A_NODE *node, int s)
{
	...
	case N_STMT_LABEL_CASE:
		prt_sem_integer((int)(long)node->llink, s+1);
...
void prt_sem_LITERAL(int lit, int s)
{
	...
	else if (literal_table[lit].type == float_type)
...
void prt_sem_A_TYPE(A_TYPE *t, int s)
{
	...
	if (t == int_type)
		...
		case T_ARRAY:
			...
			print_space(s);
			printf("| INDEX\n");
			prt_sem_integer((int)(long)t->expr, s+2);
			print_space(s);
			printf("| ELEMENT_TYPE\n");
			prt_sem_A_TYPE(t->element_type, s+2);
			break;
...
void prt_sem_A_ID(A_ID *id, int s)
{
	...
	if (id->init)
	{
		...
		{
			if (id->init)
				prt_sem_integer((int)(long)id->init, s+2);
		}
		else
