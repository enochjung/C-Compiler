...
A_ID* setFunctionDeclaratorSpecifier(A_ID* id, A_SPECIFIER* p)
{
	...
	if (id->type==NULL || id->type->kind != T_FUNC) // 전역에 i=10; 할 경우 함수로 취급. NULL 체크 추가
	{
		syntax_error(21, "");
		return id;
	}
	...
	if (a && (a->kind != ID_FUNC || a->type->expr))
		syntax_error(12, id->name); // 파라미터가 다른 케이스 체크는 생략
	a = id->type->field;
	while (a)
	{
		if (strlen(a->name))
			current_id = a;
		else if (a->type != void_type)
			syntax_error(23, "");
		a = a->link;
	}
	return id;
...
A_ID* setParameterDeclaratorSpecifier(A_ID* id, A_SPECIFIER* p)
{
	if (searchIdentifierAtCurrentLevel(id->name, id->prev))
		syntax_error(12, id->name);
	if (p->stor || (p->type==void_type && strlen(id->name)))
		syntax_error(14, "");
	setDefaultSpecifier(p);
	id = setDeclaratorElementType(id, p->type);
	id->kind = ID_PARM;
...
{
	printf("isNotSameFormalParameters start\n");
	if (a == NULL)
		return FALSE;
	while (a)
	{
	...
	printf("isNotSameFormalParameters end\n");
	return b != NULL? TRUE : FALSE;
}

BOOLEAN isNotSameType(A_TYPE* t1, A_TYPE* t2)
{
	printf("isNotSameType start\n");
...
{
	printf("isPointerOrArrayType start\n");
...
void initialize()
{
	...
	// malloc 리턴 타입 변경
	setDeclaratorTypeAndKind(makeIdentifier("malloc"), setTypeField(setTypeElementType(makeType(T_FUNC), setTypeElementType(makeType(T_POINTER), void_type)), setDeclaratorTypeAndKind(makeDummyIdentifier(), int_type, ID_PARM)), ID_FUNC);
	// 기존 코드
...
{
	++syntax_err;
	fprintf(stderr, "line %d: syntax error: ", line_no);
	switch (i)
	{
	case 11:
		fprintf(stderr, "illegal referencing struct or union identifier %s", s);
		break;
	case 12:
		fprintf(stderr, "redeclaration of identifier %s", s);
		break;
	case 13:
		fprintf(stderr, "undefined identifier %s", s);
		break;
	case 14:
		fprintf(stderr, "illegal type specifier in formal parameter");
		break;
	case 20:
		fprintf(stderr, "illegal storage class in type specifiers");
		break;
	case 21:
		fprintf(stderr, "illegal function declarator");
		break;
	case 22:
		fprintf(stderr, "conflicting parm type in prototype function %s", s);
		break;
	case 23:
		fprintf(stderr, "empty parameter name");
		break;
	case 24:
		fprintf(stderr, "illegal declaration specifiers");
		break;
	case 25:
		fprintf(stderr, "illegal function specifiers");
		break;
	case 26:
		fprintf(stderr, "illegal or conflicting return type in function %s", s);
		break;
	case 31:
		fprintf(stderr, "undefined type for identifier %s", s);
		break;
	case 32:
		fprintf(stderr, "incomplete forward reference for identifier %s", s);
		break;
	default:
